...
class MToast{
    ...
    openToast(toast) {
        ...
        for(let i = 0; i < this.stack.length; i++) {
            if(this.stack[i].element.dataset.transitionState == "complete") {
                ...
                this.stack[i].element.style.transition = this.duration + " transform " + this.timing + (this.dimOld ? ", " + " opacity ease" : "")
                if(this.dimOld) {
    ...
    _transformToast(toast) {
        if(this.position == "top-center") {
            ...
            toast.element.style.transform = `translate(50vw, 50%), -${toast.offsetY}px)`
